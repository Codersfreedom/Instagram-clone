...
const EditProfile = ({ isOpen, onClose }) => {
    ...
    const handleEditProfile  = async ()=>{
        try {
           await EditProfile(inputs,selectedImg);
            setSelectedImg(null);
            onClose();
        } catch (error) {
							...
							<Stack spacing={4} w={"full"} maxW={"md"} bg={"black"} p={6} my={0}>
								...
								<FormControl>
									<Stack direction={["column", "row"]} spacing={6}>
										<Center>
											<Avatar size='xl' src ={ selectedImg || user.profilePicURL} border={"2px solid white "} />
										</Center>
										<Center w='full'>
											<Button w='full' onClick= {()=> fileRef.current.click()} >Edit Profile Picture</Button>
										</Center>
                                            <Input type="file" hidden ref={fileRef} onChange={handleSelectImg} />
									</Stack>
								</FormControl>
